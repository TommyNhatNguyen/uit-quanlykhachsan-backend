...
class RoomInventoryLogRepository:
    ...
    def get_list_room_inventory_logs(self, page: int = 1, page_size: int = 10) -> PaginateModel[RoomInventoryLog]:
        try:
            conn = self.db.get_connection()
            cur = conn.cursor(as_dict=True)
            cur.execute("""
            ...
            return PaginateModel[RoomInventoryLog](page=page, page_size=page_size, total=total, total_pages=total_pages, data=[RoomInventoryLog(**row) for row in rows])
        except Exception as e:
            return JSONResponse({"error": str(e)}, status_code=500)
        finally:
            conn.close()
